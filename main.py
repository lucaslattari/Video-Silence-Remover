...
def create_composite_clip(video_file, clip_filename, start_time, end_time, is_debug_mode=False):
    ...
    try:
        ...
        composite_clip = CompositeVideoClip(
            [clip, text_clip]).set_duration(end_time - start_time)
...
def parse_silence_file(txt_file):
    try:
        with open(txt_file, "r") as silence_to_remove_file:
            ...
            intervals = [tuple(map(float, line.rstrip().split(",")))
...
def create_video_clips(video_file, intervals, is_debug_mode):
    ...
    for clip_id, (start_time, end_time) in enumerate(intervals):
        if start_time == 0.0 and not clips:
            ...
            clip = create_clip(video_file, clip_filename,
                               last_end_time, start_time, is_debug_mode)

            clips.append(clip)
            last_end_time = end_time

    clip = create_clip(
        video_file, f"clip{clip_id}.mp4", last_end_time, video_file.duration, is_debug_mode)
    clips.append(clip)
